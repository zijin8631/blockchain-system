...
class P2p(object):
    ...
    def run(self):
        '''
        the event loop is the program starts an infinite loop, registers some functions to the event loop(loop), 
        and calls the corresponding coroutine function when the event occurs.
        '''
        loop = asyncio.get_event_loop()
        self.loop = loop
        #when the loop run this function, end it.
...
class TCPServer(object):
    ...
    def handle_handshake(self, msg):
        ...
        data = {
            "last_height": -1,
            "genesis_block": ""#the genesis_block is the first block in the blockchain.
        }
        if genesis_block:
            data = {
...
class PeerServer(Singleton):
    ...
    def nodes_find(self, p2p_server):
        local_ip = socket.gethostbyname(socket.getfqdn(socket.gethostname()))#get the local ip address.
    ...
    #add the txs in the node which need to be broadcasted
